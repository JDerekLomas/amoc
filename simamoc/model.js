...
'  if (row >= p.ny) { return; }',
'',
'  // Bit-reversed index: use countLeadingZeros for constant-time bit count',
'  let bits = 32u - countLeadingZeros(p.nx - 1u);',
'  var rev = 0u;',
'  var val = i;',
...
'  imOut[m * ny] = 0.0;',
'',
'  for (var j = 1u; j < ny - 1u; j++) {',  // interior rows only (skip boundary ny-1)
...
'    let a = invDy2;',
'    let cp = select(0.0, invDy2, j > 1u);',  // c[j-1]: 0 at boundary j-1=0, invDy2 for interior
'    let w = a / b_prev;',
'    b_j -= w * cp;',
'    rhs_r -= w * dR_prev;',
'    rhs_i -= w * dI_prev;',
...
'  reOut[m * ny + last] = 0.0;',
'  imOut[m * ny + last] = 0.0;',
'  for (var jj = 1u; jj < ny - 1u; jj++) {',  // j = ny-2 down to 1
'    let j = last - jj;',
...
'@group(0) @binding(8) var<storage, read> ekmanSal: array<f32>;',   // stacked: [u_ek | v_ek | salClim] (3*N)
'@group(0) @binding(9) var<storage, read_write> forcing: array<f32>;', // stacked: [snow | ice | evap | precip] (4*N)
'@group(0) @binding(10) var<storage, read> atmosphere: array<f32>;', // stacked: [airTemp | moisture] (2*N)
'',
'fn idx(i: u32, j: u32) -> u32 { return j * params.nx + i; }',
'',
'@compute @workgroup_size(16, 16)',
...
'',
'  // 8. Thin cirrus: persistent ice clouds scaling with atmospheric moisture',
'  // Low albedo (nearly transparent to SW) but strong greenhouse (traps LW)',
'  let cirrus = 0.15 * humidity * clamp(1.0 - absLat / 80.0, 0.1, 1.0);',
'',
'  // Combine cloud types',
...
'  // Cirrus: albedo 0.05, greenhouse 0.20 (net warming ~+15 W/m²)',
'  // Thick convective: albedo 0.30, greenhouse 0.10 (net ~neutral)',
'  // Low clouds: albedo 0.35, greenhouse 0.03 (net cooling)',
'  let w_total = max(cirrus + thickConv + lowCloud, 0.01);',
'  let w_ci = cirrus / w_total;',
'  let w_cv = thickConv / w_total;',
'  let w_lo = lowCloud / w_total;',
'  let cloudAlbedo = cloudFrac * (w_ci * 0.05 + w_cv * 0.30 + w_lo * 0.35);',
...
'  olr *= soOlrMult;',
'  // LW greenhouse: cirrus traps most (0.20), convective moderate (0.10), low least (0.03)',
...
'  // BUG-FIX: every rate update here MUST be * params.dt. Earlier version',
'  // was missing dt on newIce growth/decay and the latent-heat tempOut term,',
'  // which let melting subtract ~0.1°/step where ice and warm SST coexist —',
'  // catastrophically cooling the planet in seconds of wall time.',
'  let T_freeze = -1.8;',
...
'    newIce = oldIce + params.dt * params.iceK * (T_freeze - tempOut[k]) * (1.0 - oldIce);',
'    // Brine rejection: freezing concentrates salt in surface water',
'    tempOut[salK] += params.dt * 0.5 * params.iceK * (T_freeze - tempOut[k]) * (1.0 - oldIce);',
'    // Ice insulates: prevent SST from dropping far below freezing',
'    tempOut[k] = max(tempOut[k], T_freeze - 2.0 * (1.0 - oldIce));',
'  } else if (oldIce > 0.001) {',
'    // Melting: ice shrinks, rate proportional to warming above freezing',
'    let meltRate = params.iceK * (tempOut[k] - T_freeze) * oldIce;',
'    newIce = oldIce - params.dt * meltRate;',
'    // Meltwater freshens surface',
'    tempOut[salK] -= params.dt * 0.5 * meltRate;',
'    // Melting absorbs latent heat, cooling the surface',
'    tempOut[k] -= params.dt * 0.3 * meltRate;',
'  }',
'  forcing[k + N_ek] = clamp(newIce, 0.0, 1.0);',  // write ice back to forcing buffer
'}'
].join('\n');

// ── ATMOSPHERE COMPUTE SHADER ──
// Prognostic 1-layer atmosphere: airTemp + moisture (stacked layout)
// Evolves via diffusion, surface exchange, evaporation, condensation, latent heat
var atmosphereShaderCode = [
'struct Params {',
'  nx: u32, ny: u32,',
'  dx: f32, dy: f32,',
'  dt: f32, beta: f32,',
...
'@group(0) @binding(3) var<storage, read> atmIn: array<f32>;',
'@group(0) @binding(4) var<storage, read_write> atmOut: array<f32>;',
'@group(0) @binding(5) var<storage, read> ekmanVel: array<f32>;',
'',
'fn idx(i: u32, j: u32) -> u32 { return j * params.nx + i; }',
'fn qSat(T: f32) -> f32 { return 3.75e-3 * exp(0.067 * T); }',
'',
'@compute @workgroup_size(16, 16)',
'fn main(@builtin(global_invocation_id) id: vec3u) {',
'  let i = id.x;',
...
'  let N = nx * ny;',
'',
'  // Polar boundaries: copy from neighbor row',
'  if (j < 1u || j >= ny - 1u) {',
'    let src = select(idx(i, ny - 2u), idx(i, 1u), j == 0u);',
'    atmOut[k] = atmIn[src];',
'    atmOut[k + N] = atmIn[src + N];',
'    return;',
'  }',
'',
'  // Periodic wrapping in x',
...
'  let kn = idx(i, j + 1u); let ks = idx(i, j - 1u);',
'',
'  // Grid metric. The atmosphere shader USED to multiply dx by cosLat for',
'  // a spherical correction, but with cosLat clamped to 0.087 that made',
'  // invDx² ≈ 1.4e8 at high lats on the 1024×512 grid → the polar',
'  // Laplacian violated CFL (~21°/step from a 1° gradient) → airTemp',
'  // explodes to NaN within ~600 steps. The temperature shader uses',
'  // dx directly without cosLat, so atmosphere now matches it.',
'  let lat = -79.5 + f32(j) / f32(ny - 1u) * 159.0;',
'  let latRad = lat * 3.14159265 / 180.0;',
...
'  let invDy2 = invDy * invDy;',
'',
'  // Air temperature: diffusion + wind advection',
'  let aE = atmIn[ke]; let aW = atmIn[kw];',
'  let aN = atmIn[kn]; let aS = atmIn[ks];',
'  let lapAir = invDx2 * (aE + aW - 2.0 * atmIn[k]) + invDy2 * (aN + aS - 2.0 * atmIn[k]);',
'  let airDiff = params.kappaAtm * lapAir;',
'',
'  // Wind advection (Ekman velocities as proxy for low-level atmospheric flow)',
'  let u_ek = ekmanVel[k] * params.windStrength;',
'  let v_ek = ekmanVel[k + N] * params.windStrength;',
'  let dAdx = (aE - aW) * 0.5 * invDx;',
'  let dAdy = (aN - aS) * 0.5 * invDy;',
'  let airAdvec = u_ek * dAdx + v_ek * dAdy;',
'',
'  // Moisture: diffusion + wind advection',
'  let qE = atmIn[ke + N]; let qW = atmIn[kw + N];',
'  let qN = atmIn[kn + N]; let qS = atmIn[ks + N];',
'  let lapQ = invDx2 * (qE + qW - 2.0 * atmIn[k + N]) + invDy2 * (qN + qS - 2.0 * atmIn[k + N]);',
'  let qDiff = params.kappaAtm * lapQ;',
'  let dQdx = (qE - qW) * 0.5 * invDx;',
'  let dQdy = (qN - qS) * 0.5 * invDy;',
'  let moistAdvec = u_ek * dQdx + v_ek * dQdy;',
'',
'  // Surface temperature and exchange rate',
'  var surfT: f32;',
'  var gamma: f32;',
'  if (mask[k] != 0u) {',
'    surfT = oceanTemp[k];',
'    gamma = params.gammaOA;',
'  } else {',
'    let yearPhase = 2.0 * 3.14159265 * (params.simTime % 10.0) / 10.0;',
'    let declination = 23.44 * sin(yearPhase) * 3.14159265 / 180.0;',
'    let cosZenith = cos(latRad) * cos(declination) + sin(latRad) * sin(declination);',
'    surfT = 50.0 * max(0.0, cosZenith) - 20.0;',
'    gamma = 0.01;',
'  }',
'  let exchange = gamma * (surfT - atmIn[k]);',
'',
'  // Evaporation: ocean only, from saturation deficit',
'  var evap: f32 = 0.0;',
'  if (mask[k] != 0u) {',
'    let qs = qSat(surfT);',
'    let deficit = max(0.0, qs - atmIn[k + N]);',
'    evap = params.evapCoeff * deficit;',
'  }',
'',
'  // Update moisture: advection + diffusion + evaporation',
'  // BUG-FIX: evap is a per-time-unit rate; must be multiplied by dt.',
'  // Without this, moisture grew ~1.0 per step regardless of dt — runaway.',
'  var newQ = atmIn[k + N] + params.dt * (-moistAdvec + qDiff + evap);',
'',
'  // Condensation: excess above saturation precipitates',
'  let qs_air = qSat(atmIn[k]);',
'  var precip: f32 = 0.0;',
'  if (newQ > qs_air) {',
'    precip = newQ - qs_air;',
'    newQ = qs_air;',
...
'  // BUG-FIX: latent heat must scale by dt — otherwise airTemp grew',
'  // hundreds of degrees per simulated second, propagating into the',
'  // temperature shader via cloud-albedo + OLR feedback.',
'  let latentHeat = 800.0 * precip;',
'',
'  // Update air temperature: advection + diffusion + surface exchange + latent heat',
'  atmOut[k] = atmIn[k] + params.dt * (-airAdvec + airDiff + exchange + latentHeat);',
'  atmOut[k + N] = newQ;',
'}'
].join('\n');
